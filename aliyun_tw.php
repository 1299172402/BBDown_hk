...
function handler($request, $context): Response
{
    ...
    */
    ...
    $upstream_web_interface_view = 'https://api.bilibili.com/x/web-interface/view';
    $upstream_pgc_view_web_season = 'https://api.bilibili.com/pgc/view/web/season';
    $upstream_biliapi_net_playurl = 'https://grpc.biliapi.net/bilibili.app.playurl.v1.PlayURL/PlayView';
    $upstream_bilibili_com_playurl = 'https://app.bilibili.com/bilibili.pgc.gateway.player.v1.PlayURL/PlayView';
    $timeout = 5; // seconds
    ...
    }elseif (substr_count($request_uri,'/web-interface/view')!=0){
        $url = $upstream_web_interface_view . '?' .$request_query;
        curl_setopt($ch, CURLOPT_REFERER, $req_referer);
    }elseif (substr_count($request_uri,'/pgc/view/web/season')!=0){
        $url = $upstream_pgc_view_web_season . '?' .$request_query;
        curl_setopt($ch, CURLOPT_REFERER, $req_referer);
    }elseif (substr_count($request_uri,'/bilibili.app.playurl.v1.PlayURL/PlayView')!=0){
        $url = $upstream_biliapi_net_playurl . '?' .$request_query;
        curl_setopt($ch, CURLOPT_REFERER, $req_referer);
    }elseif (substr_count($request_uri,'/bilibili.pgc.gateway.player.v1.PlayURL/PlayView')!=0){
        $url = $upstream_bilibili_com_playurl . '?' .$request_query;
        curl_setopt($ch, CURLOPT_REFERER, $req_referer);
    }else{
